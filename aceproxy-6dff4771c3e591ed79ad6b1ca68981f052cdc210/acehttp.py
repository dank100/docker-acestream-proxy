...
class HTTPHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    channels = [
        ('DanishBay', '88994d418b6cf4b6052dce047abcb71110b0e7e7',
         'http://185.53.131.158:8000/pid/88994d418b6cf4b6052dce047abcb71110b0e7e7/Danishbay.mp4')
    ]
        ...
        for q in self.queryList:
            ...
            for li in results:
                ...
                self.channels.append(
                    (str(name), str(pid), self.server + "/pid/" + str(pid) + "/" + str(name).replace(" ", "") + ".mp4"))
    ...
    def findPID(self, q):
        ...
        content = response.read()
        soup = BeautifulSoup(page.content, 'html.parser')
        span = soup.find('span', _class="pull-right glyphicon glyphicon-copy js-tooltip js-copy")
        return self.server + "/pid/" + span.data - copy + str(q) + ".mp4"
    ...
    def do_GET(self, headers_only=False):
        ...
        try:
            ...
            if self.reqtype == 'm3u':
                ...
                self.send_header("Content-Type", "m3u")
        ...
        try:
            ...
            self.path = findPID()   #Find PID from API
            #self.dieWithError(400)  # 400 Bad Request
            return
